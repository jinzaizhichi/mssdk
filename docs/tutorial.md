...
[mssdk](https://github.com/cdmaxsmart/mssdk) 数据接口一览表
...
```python
import mssdk as ms
get_roll_yield_bar_df = ms.get_roll_yield_bar(type_method="date", var="RB", start_day="20180618", end_day="20180718", plot=True)
...
```python
import mssdk as ms
zdzk_fund_index_df = ms.zdzk_fund_index(index_type=32, plot=True)
