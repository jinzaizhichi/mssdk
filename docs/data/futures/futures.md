## [mssdk](https://github.com/msfamily/mssdk) 期货数据
...
```python
import mssdk as ms
futures_comm_info_df = ms.futures_comm_info(symbol="所有")
...
| 名称   | 类型 | 描述        |
| -------- | ---- |  --- |
| trade_date | str  |   trade_date="20200713"; 需要指定为交易日, 且是近期的日期|
...
![](https://jfds-1252952517.cos.ap-chengdu.myqcloud.com/mssdk/readme/inventory/shfe_cu.jpg)
...
在 [mssdk](https://github.com/msfamily/mssdk) 中可以通过 **get_roll_yield_bar** 接口下载展期收益率数据.
...
基差是商品期货非常重要的基本面因素, mssdk 在这里提供 3 中获取基差的方法: 
...
0   NYBOT-棉花     88.617  12648.633415  ...     88.610  16:26:09  2021-07-21
1    LME镍3个月  18405.500  119162.72865  ...  18420.000  16:25:22  2021-07-21
2    LME铅3个月   2321.950  15033.000885  ...   2322.500  16:27:02  2021-07-21
3    LME锡3个月  33427.000   216416.4261  ...  33440.000  15:58:20  2021-07-21
4    LME锌3个月   2937.850  19020.522255  ...   2940.500  16:26:49  2021-07-21
5    LME铝3个月   2446.400   15838.72752  ...   2445.500  16:27:07  2021-07-21
6    LME铜3个月   9336.650  60448.273095  ...   9336.500  16:27:09  2021-07-21
7    CBOT-黄豆   1384.275   3293.048156  ...   1385.500  16:26:57  2021-07-21
8    CBOT-小麦    707.850   1683.902503  ...    707.750  16:27:06  2021-07-21
9    CBOT-玉米    569.450   1451.424757  ...    569.500  16:27:10  2021-07-21
10  CBOT-黄豆油     63.118   9009.066476  ...     63.160  16:26:45  2021-07-21
11  CBOT-黄豆粉    371.830   2654.091213  ...    371.900  16:26:47  2021-07-21
13    COMEX铜    426.485  60873.787444  ...    426.500  16:27:10  2021-07-21
14  NYMEX天然气      3.938     25.495793  ...      3.936  16:27:02  2021-07-21
15   NYMEX原油     67.778   3203.350269  ...     67.780  16:27:19  2021-07-21
16   COMEX白银     25.099    162.498456  ...     25.090  16:27:17  2021-07-21
17   COMEX黄金   1803.900    375.479521  ...   1804.000  16:27:18  2021-07-21
18   CME-瘦肉猪     91.177    590.307251  ...     91.200  02:04:59  2021-07-21
19     布伦特原油     69.908    3304.01916  ...     69.920  16:27:18  2021-07-21
20       伦敦金   1803.740    375.446217  ...   1803.940  16:27:00  2021-07-21
21       伦敦银     25.030      5.209963  ...     25.050  16:27:00  2021-07-21
22      伦敦铂金   1067.220    222.140504  ...   1067.100  16:26:44  2021-07-21
23      伦敦钯金   2654.400    552.510029  ...   2656.500  16:26:15  2021-07-21
24       马棕油   4114.900    856.511271  ...   4119.000  16:27:19  2021-07-21
```
...
```python
import mssdk as ms
futures_sgx_daily_df = ms.futures_sgx_daily(trade_date="20200306", recent_day="2")
...
```python
import mssdk as ms
futures_main_sina_hist = ms.futures_main_sina(symbol="V0", trade_date="20210817")
...
4     2009-06-02  6590  6590  6545  6555   13496    9388      0
          ...   ...   ...   ...   ...     ...     ...    ...
2965  2021-08-11  9205  9330  9175  9325  277951  296837   9230
2966  2021-08-12  9325  9365  9280  9335  239504  290659   9320
2967  2021-08-13  9320  9460  9290  9335  343020  269624   9365
2968  2021-08-16  9305  9435  9300  9330  265546  243547   9355
2969  2021-08-17  9305  9480  9300  9450  265346  232809   9380
```
...
```python
import mssdk as ms
futures_index_cfmmc_df = ms.futures_index_cscidx(index_name="林木综合指数", start_date="2010-01-01", end_date="2020-04-06")
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="猪肉批发价")
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="仔猪价格")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="生猪期货指数")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="二元母猪价格")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="生猪产能数据")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="饲料原料数据")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="中央储备冻猪肉")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="白条肉")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="育肥猪配合饲料")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="肉类价格指数")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="猪粮比价")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="猪企销售简报-销售量")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="猪企销售简报-销售额")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_info_df = ms.futures_pig_info(symbol="猪企销售简报-销售均价")
print(futures_pig_info_df)
```
...
```python
import mssdk as ms
futures_pig_rank_df = ms.futures_pig_rank(symbol="外三元")
...
```python
import mssdk as ms
futures_pig_rank_df = ms.futures_pig_rank(symbol="内三元")
print(futures_pig_rank_df)
```
...
```python
import mssdk as ms
futures_pig_rank_df = ms.futures_pig_rank(symbol="土杂猪")
print(futures_pig_rank_df)
```
...
```python
import mssdk as ms
futures_pig_rank_df = ms.futures_pig_rank(symbol="玉米")
print(futures_pig_rank_df)
```
...
```python
import mssdk as ms
futures_pig_rank_df = ms.futures_pig_rank(symbol="豆粕")
print(futures_pig_rank_df)
```
...
```python
import mssdk as ms
futures_egg_price_yearly_df = ms.futures_egg_price_yearly()
...
```python
import mssdk as ms
futures_egg_price_df = ms.futures_egg_price()
...
```python
import mssdk as ms
futures_egg_price_area_df = ms.futures_egg_price_area()
