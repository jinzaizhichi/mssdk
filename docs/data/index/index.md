...
```python
import mssdk as ms
index_investing_global_df = ms.index_investing_global(country="美国", index_name="VIX恐慌指数", period="每月", start_date="2005-01-01", end_date="2020-06-05")
...
```python
import mssdk as ms
spot_df = ms.spot_goods(symbol="波罗的海干散货指数")
print(spot_df)
```
...
```python
import mssdk as ms
index_vix_df = ms.index_vix(start_date="2020-06-11", end_date="2020-06-11")  # 只能获取当前交易日近一个月内的数据
print(index_vix_df)
```
