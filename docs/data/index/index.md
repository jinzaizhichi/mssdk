## [mssdk](https://github.com/cdmaxsmart/mssdk) 指数数据
...
股票指数信息一览表(可以在 msShare 中通过如下代码获取本表)
...
4. 在安装 [mssdk](https://github.com/cdmaxsmart/mssdk) 后输入, 如 **ms.index_investing_global(country="美国", index_name="VIX恐慌指数", period="每月", start_date="2005-01-01", end_date="2020-06-05")**;
5. 稍后就可以获得所需数据.
...
| index_name | str  | Y    |  index_name="美元指数"; 可以通过 ms.index_investing_global_country_name_url("美国") 获取|
...
```python
import mssdk as ms
index_investing_global_df = ms.index_investing_global(country="美国", index_name="VIX恐慌指数", period="每月", start_date="20050101", end_date="20200605")
...
```python
import mssdk as ms
df_index = ms.weibo_index(word="期货", time_type="3month")
print(df_index)
```
...
![cookies](https://jfds-1252952517.cos.ap-chengdu.myqcloud.com/msshare/readme/index_baidu/cookies.png)
...
![](https://jfds-1252952517.cos.ap-chengdu.myqcloud.com/msshare/readme/index/clipboard_20191204050206.png)
...
![](https://jfds-1252952517.cos.ap-chengdu.myqcloud.com/msshare/readme/index/clipboard_20191211034449.png)
...
```python
import mssdk as ms
sw_index_df = ms.sw_index_cons(index_code="801010")
print(sw_index_df)
```
...
```python
import mssdk as ms
sw_index_df = ms.sw_index_daily(index_code="801010", start_date="2019-12-01", end_date="2019-12-07")
print(sw_index_df)
```
...
```python
import mssdk as ms
sw_index_df = ms.sw_index_daily_indicator(index_code="801010", start_date="2019-12-01", end_date="2019-12-07", data_type="Day")
print(sw_index_df)
```
...
```python
import mssdk as ms
sw_index_df = ms.sw_index_daily_indicator(index_code="801010", start_date="2019-12-01", end_date="2019-12-07", data_type="Week")
print(sw_index_df)
```
...
| 名称   | 类型 | 必选 | 描述    |
| -------- | ---- | ---- | --- |
| symbol | str  | Y   | symbol="柯桥时尚指数"; choice of {'柯桥时尚指数', '时尚创意指数', '时尚设计人才数', '新花型推出数', '创意产品成交数', '创意企业数量', '时尚活跃度指数', '电商运行数', '时尚平台拓展数', '新产品销售额占比', '企业合作占比', '品牌传播费用', '时尚推广度指数', '国际交流合作次数', '企业参展次数', '外商驻点数量变化', '时尚评价指数'} |
...
目标地址: https://www.drewry.co.uk/supply-chain-advisors/supply-chain-expertise/world-container-index-assessed-by-drewry

描述: 获取 Drewry 集装箱指数的数据; 该接口返回一年内的数据
...
```python
import mssdk as ms
drewry_wci_index_df = ms.drewry_wci_index()
...
4  2020-06-04  1686.7
..         ...      
46 2021-03-25  4871.7
47 2021-04-01  4883.4
48 2021-04-08  4910.5
49 2021-04-15  4904.7
50 2021-04-22  4913.1
51 2021-04-29  4983.7
52 2021-05-06  5472.3
```
...
```python
import mssdk as ms
stock_zh_index_hist_csindex_df = ms.stock_zh_index_hist_csindex(symbol="H30374")
...
```
            date     close
0     2016-07-04   5386.28
1     2016-07-05   5337.20
2     2016-07-06   5304.66
3     2016-07-07   5352.74
4     2016-07-08   5345.67
          ...       ...
1293  2021-06-28  12803.58
1294  2021-06-29  12816.58
1295  2021-06-30  12652.33
1296  2021-07-01  12528.69
1297  2021-07-02  12223.05
```
