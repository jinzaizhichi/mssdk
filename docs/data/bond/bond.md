...
```python
import mssdk as ms
bond_convert_jsl_df = ms.bond_cov_jsl()
...
```python
import mssdk as ms
bond_futures_deliverable_coupons_df = ms.bond_futures_deliverable_coupons(trade_date="2020-09-22")
...
| 名称   | 类型 | 必选 | 描述       |
| -------- | ---- | ---- | --- |
| symbol | str  | Y    |   symbol="政策性金融债(进出口行)"; 通过网页查询或调用 **ms.bond_china_close_return_map** 获取|
...
```python
import mssdk as ms
bond_china_close_return_df = ms.bond_china_close_return(symbol="政策性金融债(进出口行)", start_date="2020-08-30", end_date="2020-09-30")
